    ...
    <header class="h-20 border-b border-white/5 flex items-center justify-between px-8 bg-[#303841]/80 backdrop-blur-xl sticky top-0 z-20 shrink-0">
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="relative dropdown-container" [class.open]="isDropdownOpen">
          <div (click)="toggleDropdown()" class="flex items-center gap-3 pl-2 cursor-pointer hover:bg-white/5 p-2 rounded-lg transition">
          ...
          <div *ngIf="isDropdownOpen" class="absolute right-0 mt-2 w-48 glass-panel border border-white/10 rounded-xl shadow-xl overflow-hidden z-50">
            <a [routerLink]="['/profile']" (click)="closeDropdown()" class="block px-4 py-3 text-sm text-gray-300 hover:bg-white/5 hover:text-white transition cursor-pointer border-b border-white/10 last:border-b-0">
              <svg class="w-4 h-4 inline mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
              </svg>
              Profile
            </a>
            <button (click)="logout()" class="w-full text-left px-4 py-3 text-sm text-gray-300 hover:bg-red-500/20 hover:text-red-400 transition cursor-pointer">
              <svg class="w-4 h-4 inline mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
